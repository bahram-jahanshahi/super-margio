...
@Service
public class MergeTwoHomogeneousListsService implements MergeTwoHomogeneousListsUseCase {
    @Override
    public void execute() {
    ...
    private <T, S> List<S> merge(Supplier<List<T>> listSupplier1, Supplier<List<T>> listSupplier2, Function<List<List<T>>, List<S>> merger) {
    ...
    private static Supplier<List<Person>> people2() {
        return () -> List.of(new Person(4L, "David", 28),
                new Person(5L, "Eve", 22),
                new Person(6L, "Frank", 40));
    }

    private static Supplier<List<Person>> people1() {
